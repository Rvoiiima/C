...
for(int l=0;l<900;l++){
	 ...
	 for(i=0;i<=X;i++) {
	 	...
	 	for(m = -2; m<3 ; m++){
	 		...
	 		y += A[m+2]* pow(M_PI,2) * sin(M_PI*(x-m)) /(M_PI*(x-m))*cos(M_PI*(x-m))/(pow(M_PI,2)-4*pow(M_PI,2)*pow((x-m),2));
	 		y2 += A2[m+2]* pow(M_PI,2) * sin(M_PI*(x-m)) /(M_PI*(x-m))*cos(M_PI*(x-m))/(pow(M_PI,2)-4*pow(M_PI,2)*pow((x-m),2));	 		
        }
	 fprintf(data,"%f %f\n",y,y2);	 
